...
|----------|------|--------|------|
| `query` | **필수** | - | 사용자 질문 |
| `messages` | 선택 | `null` | 멀티턴 대화 히스토리 `[{role, content}]` |
| `stream` | 선택 | `false` | SSE 실시간 스트리밍 |
| `agent_type` | 선택 | `null` | 에이전트 강제 지정 (14개) |
| `user_id` | 선택 | `null` | 트레이싱용 사용자 ID |
| `filters` | 선택 | `null` | 날짜 필터 직접 지정 |

> 인증: `.env`의 `OPENAI_COMPAT_API_KEY` 설정 시 Bearer 토큰 필수. 미설정 시 인증 없음.
> `/api/chat`과 `/v1/chat/completions` 모두 동일한 키 사용.
...
대화형 모드 명령어: `/new`(새 대화), `/history`(히스토리 보기), `/use <agent>`, `/help`
