...
#### 1. analytics — 통계/편수/목록 조회
논문 편수, 목록, 추이 등 **숫자 기반 집계**를 처리합니다. MariaDB SQL로 정확한 수치를 제공합니다.

**이 에이전트로 분류되는 키워드:** "편수", "건수", "몇 편", "목록", "리스트", "제목", "통계", "추이", "그래프"
...
**데이터 소스:** MariaDB SQL만 사용 (Milvus 미사용)
**특이사항:** 키워드 검색 시 0건이면 자동으로 키워드 없이 재시도합니다.
...
| 에이전트 | 응답 형식 |
|----------|----------|
| analytics | 표(table) 형식으로 편수/목록 제공 |
